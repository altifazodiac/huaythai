...
export default function AdminCreditTopupPage() {
  ...
  // ให้เหลือ useEffect เฉพาะ selectedUser
  useEffect(() => {
  ...
  const handleTopup = async () => {
    if (!securityVerified) {
      setShowSecurityFeatures(true); // เด้ง popup เฉพาะตอนกดปุ่ม
      return;
    }

    setLoading(true);
    
    // Security delay for banking standard
    ...
    if (updateError) {
      ...
      toast.success("เติมเครดิตสำเร็จ!", {
        description: `เพิ่มเครดิต ฿${Number(amount).toLocaleString()} ให้ ${updatedUser?.name || updatedUser?.email}. ยอดคงเหลือใหม่ ฿${(newBalance as number).toLocaleString()}`,
        duration: 5000
      });
  ...
  return (
    <SecurityMiddleware 
      ...
      onSecurityPass={() => {
        ...
        setShowSecurityFeatures(false); // ปิด popup หลังผ่าน security
      }}
      requireCaptcha={showSecurityFeatures}
    >
          ...
          <CardContent className="space-y-6">
            <div className="space-y-4">
              ...
              {selectedUserData && (
                <div className="space-y-4">
                  ...
                  <CreditBalanceDisplay
                    balance={selectedUserData.credit_balance}
                    size="md"
                    variant="default"
                    animated={true}
                    showDetails={true}
              ...
              <Button
                ...
                onClick={handleTopup}
                disabled={loading || !selectedUser || !amount || !securityVerified}
              >
                {loading ? (
