...
export async function fetchDashboardData(dateRange: string = 'week'): Promise<DashboardData> {
  try {
    ...
    // Create results map for winnings calculations
    const resultsMap = await createResultsMap(supabase, previousStartDate.toISOString())
...
async function fetchTicketsData(startDate: Date, previousStartDate: Date, resultsMap: Record<string, any>) {
  ...
  const previousPeriodTickets = tickets?.filter(t => 
    new Date(t.created_at) >= previousStartDate && 
    new Date(t.created_at) < startDate
  ) || []
  ...
  for (let i = 0; i < daysBack; i++) {
    ...
    const dayRevenue = dayTickets.reduce((sum, ticket) => sum + Number(ticket.total_amount), 0)
    const dayTicketCount = dayTickets.length
    
    // Calculate payouts using proper lottery calculations
  ...
  const previousRevenue = previousPeriodTickets.reduce((sum, t) => sum + Number(t.total_amount), 0)
...
function calculateTypeDistribution(tickets: any[]) {
  ...
  tickets.forEach(ticket => {
    ticket.lottery_ticket_items?.forEach((item: any) => {
      const typeName = item.lottery_sub_types?.name || 'Unknown'
      if (!typeMap[typeName]) {
        typeMap[typeName] = {
          type: typeName,
          count: 0,
      ...
      typeMap[typeName].revenue += Number(item.amount)
    })
  })
...
async function fetchCreditData(startDate: Date, previousStartDate: Date) {
  ...
  const totalAmount = currentPeriodTx.reduce((sum, tx) => sum + Number(tx.amount), 0)
  const totalTopups = currentPeriodTx.filter(tx => tx.transaction_type === 'topup').reduce((sum, tx) => sum + Number(tx.amount), 0)
  const totalWithdrawals = currentPeriodTx.filter(tx => tx.transaction_type === 'withdrawal').reduce((sum, tx) => sum + Number(tx.amount), 0)

  // Calculate daily transactions
  const dailyTransactions = [] // Implementation needed
  
  // Calculate transaction types
  const transactionTypes = [] // Implementation needed
