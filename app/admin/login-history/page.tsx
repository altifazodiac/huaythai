...
export default function LoginHistoryPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto py-10 px-4">
      <h1 className="text-3xl font-bold mb-6 text-green-800">ประวัติการเข้าสู่ระบบ</h1>
      {loading ? (
        <div className="flex items-center justify-center py-10 text-green-700">
        ...
        <div className="overflow-x-auto rounded-lg shadow-lg bg-white/80">
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-gradient-to-r from-green-700 to-lime-400 text-white">
            ...
            <tbody>
              {history.map((item, idx) => (
                <tr key={item.id} className={idx === 0 ? "bg-lime-100 font-bold" : "hover:bg-green-50"}>
                  ...
                  <td className="py-2 px-4 flex items-center gap-2">
                    <MonitorSmartphone className="w-4 h-4 text-green-700" />
                    <span className="truncate max-w-[120px]">{item.user_agent?.slice(0, 40) || "-"}</span>
                  </td>
                  <td className="py-2 px-4">
                    {idx === 0 ? (
                      <span className="inline-block px-2 py-1 bg-lime-400 text-green-900 rounded-full text-xs animate-pulse">ล่าสุด</span>
                    ) : (
