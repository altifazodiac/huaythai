...
const createColumns = (
  ...
  },
  {
    ...
    header: "ประเภทหวย",
    cell: ({ row }: { row: Row<LotteryTicket> }) => {
      // Fix: Get all unique lottery types from ticket items
...
export default function LotteryPurchasePage() {
  ...
  // Group Bet Details - Fixed calculation logic
  const createGroups = (ticketItems: LotteryTicketItem[]) => {
    if (!ticketItems || ticketItems.length === 0) return new Map();
    
    // Get all unique type labels for each digit
    ...
    ticketItems.forEach((item) => {
      ...
      if (!digit) return; // Skip if no digit number
      
      if (!allTypeLabels[digit]) {
        allTypeLabels[digit] = [];
      }
      if (!allTypeLabels[digit].includes(label)) {
        allTypeLabels[digit].push(label);
      }
    });

    // Sort type labels for consistent display
    Object.keys(allTypeLabels).forEach((digit) => {
      const digitNum = Number(digit);
      if ([3, 4].includes(digitNum)) {
        const labels = allTypeLabels[digitNum];
        const sortedLabels = ["เต็ง", "โต๊ด"].filter(l => labels.includes(l));
        allTypeLabels[digitNum] = sortedLabels.length > 0 ? sortedLabels : labels;
      } else if (digitNum === 2) {
        const labels = allTypeLabels[digitNum];
        const sortedLabels = ["บน", "ล่าง"].filter(l => labels.includes(l));
        allTypeLabels[digitNum] = sortedLabels.length > 0 ? sortedLabels : labels;
      }
    });
    ...
    // Group by digit, numbers, and type combination
    ticketItems.forEach((item) => {
      if (!item.lottery_sub_number?.digit_number || !item.numbers) return;
      
      const digit = item.lottery_sub_number.digit_number;
      const typeLabel = item.lottery_sub_number.type_number || "-";
      const numbersStr = item.numbers.join(",");
      
      const key = `${digit}-${numbersStr}`;
      
      if (!groups.has(key)) {
        const typeLabels = allTypeLabels[digit] || [typeLabel];
        groups.set(key, {
          digit_number: digit,
          numbers: [...item.numbers],
          typeLabels: typeLabels,
          amounts: {},
          typeOrder: typeLabels,
        });
      }
      
      const group = groups.get(key)!;
      // Fix: Use actual amount from item instead of calculating
      group.amounts[typeLabel] = (group.amounts[typeLabel] || 0) + (item.amount || 0);
    });
                                              ...
                                              <div key={idx} className="flex gap-4 mb-2">
                                                <div className="w-32">
                                                  ...
                                                  <div className="text-xs font-medium">
                                                    {group.typeLabels
                                                      .map((label: string) => {
                                                        const amount = group.amounts[label] || 0;
