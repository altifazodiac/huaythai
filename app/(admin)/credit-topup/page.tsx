  ...
  const handleTopup = async () => {
    ...
    if (updateError) {
      ...
      toast.success("เติมเครดิตสำเร็จ!", {
        description: `เพิ่มเครดิต ฿${Number(amount).toLocaleString()} ให้ ${user?.name || user?.email}`,
        duration: 5000
      });
      
      // Refresh data
      const { data: updatedUsers } = await supabase.from("profiles").select("id, name, email, phone, credit_balance");
    ...
    >
      <div className="container mx-auto p-6 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-gray-600">จัดการเครดิตผู้ใช้ด้วยระบบความปลอดภัยระดับธนาคาร</p>
        </div>
