    ...
    const matchingTicketsHtml = matchingTickets && matchingTickets.length > 0 
      ? `
        <div class="matching-tickets">
          <table>
            ${(() => {
              // Group matching tickets by amount_display
              const groupedByAmount = matchingTickets.reduce((acc, ticket) => {
                if (!acc[ticket.amount_display]) {
                  acc[ticket.amount_display] = [];
                }
                acc[ticket.amount_display].push(ticket);
                return acc;
              }, {} as Record<string, typeof matchingTickets>);

              // Create rows for each amount group
              return Object.entries(groupedByAmount)
                .map(([amount, tickets]) => `
                  <tr>
                    <td style="text-align: center;width: 75px;">
                      <div style="font-size: 14px; background-color:rgb(65, 202, 76); color: white; padding: 2px 5px; border-radius: 5px;font-weight: 600;"> 3 ตัว</div>
                      <div style="font-size: 12px; color: rgb(228, 43, 43);">บน x โต๊ด</div>
                      <div style="font-size: 12px;">${amount}</div>
                    </td>
                    <td><div style="font-size: 14px; border: 1px solid rgb(56, 190, 243); padding: 2px 5px; border-radius: 5px;height: 45px;overflow-y: auto;background-color:rgb(245, 245, 245);width: 420px;">
                      ${tickets.map(t => t.teng_number).join(', ')}
                    </div></td>
