-- Quick Fix for Soft Delete Lottery Tickets
-- This script only creates the necessary function and table for soft delete
-- Run this in Supabase Dashboard SQL Editor

-- Step 1: Create soft_delete_lottery_ticket function
DO $$
BEGIN
    IF NOT EXISTS (
    ...
    ELSE
        RAISE NOTICE 'ℹ️  Function soft_delete_lottery_ticket already exists.';
        RETURN;
    END IF;
END $$;

-- Create the function (outside of DO block)
...
DO $$
BEGIN
    RAISE NOTICE '🎉 Soft delete setup completed!';
    RAISE NOTICE 'You can now use: SELECT public.soft_delete_lottery_ticket(ticket_id, reason)';
    RAISE NOTICE 'This function bypasses the draw_date validation trigger';
END $$;
