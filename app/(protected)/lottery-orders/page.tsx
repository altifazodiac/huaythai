...
const LotteryOrderPage = () => {
  ...
  const { credit, user, fetchCredit } = useAuth();
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      const { error: updateCreditError } = await supabase
        .from("profiles")
        .update({ credit_balance: currentCredit - totalPayment })
        .eq("id", user.id);
      ...
      const { data: ticketData, error: ticketError } = await supabase
        ...
        .insert({
          user_id: user.id,
          draw_date: selectedDraw?.date?.toISOString().slice(0, 10),
      ...
      const ticketItems = orders.map(order => ({
        ticket_id: ticket.id,
        lottery_sub_type_id: subTypeObj?.lottery_sub_type_id,
        lottery_sub_number_id: order.prizeId,
        numbers: [order.numbers],
        amount: parseFloat(orderPrices[order.id]?.amount || "0"),
      }));
    ...
    <div className="min-h-screen bg-background font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          ...
          <div className="lg:col-span-3 bg-card text-card-foreground p-4 rounded-lg shadow-md flex flex-col">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold text-foreground">{orders.length} รายการ</h2>
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="sm" onClick={() => setConfirmDeleteAll(true)} disabled={orders.length === 0} className="text-red-600 hover:text-red-800 disabled:text-muted-foreground disabled:cursor-not-allowed">
                  ลบทั้งหมด <Badge variant="destructive" className="ml-2">{orders.length}</Badge>
                </Button>
                <Button variant="ghost" size="sm" onClick={deleteLastOrder} disabled={orders.length === 0} className="text-red-600 hover:text-red-800 ml-2 disabled:text-muted-foreground disabled:cursor-not-allowed">
                  ลบล่าสุด <Badge variant="destructive" className="ml-2">{orders.length > 0 ? 1 : 0}</Badge>
                </Button>
                <Button 
                  className="group flex items-center gap-2 bg-red-600 text-white hover:bg-red-700 transition-all duration-300 ease-in-out hover:scale-105 disabled:cursor-not-allowed disabled:bg-gray-400 disabled:hover:bg-gray-400"
                  onClick={() => setShowRightPanel(false)}
                  disabled={orders.length === 0}
                >
                  <FaMoneyBill className="h-8 w-8 transition-transform duration-300 group-hover:rotate-90" />
                  ใส่ราคาหวย
                </Button>
              </div>
            </div>
            ...
            <div
              ...
              className="flex flex-row gap-4 overflow-x-auto max-w-full pb-2 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700 scrollbar-track-transparent"
              ...
              <AnimatePresence>
                {(() => {
                  ...
                  if (uniquePrizeIdsInOrders.length === 0) {
                    return (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-10 text-muted-foreground w-full">
                        ยังไม่มีรายการ
                      </motion.div>
                    );
                  }
                  ...
                  return uniquePrizeIdsInOrders.map((prizeId, idx) => {
                    ...
                    return (
                      <motion.div
                        ...
                        exit="exit"
                      >
                        <Card className="shadow-lg border border-red-200 dark:border-red-900">
                          <CardHeader
                            className="flex flex-row items-center justify-between space-y-0 pb-1 p-2 rounded-t-lg bg-gradient-to-r from-red-900 to-red-700 min-h-0"
                          >
                            <div className="flex flex-col items-start">
                              ...
                              <Badge variant="destructive" className="mt-1 text-xs bg-white/80 text-red-700 dark:bg-black/50 dark:text-red-400 border-none px-2 py-0.5 rounded-full">
                          ...
                          <CardContent className="p-0 max-h-[calc(90vh-320px)] overflow-y-auto">
                            ...
                            <AnimatePresence>
                              {!showRightPanel && (
                                ...
                                >
                                  <div className="p-3">
                                    <label className="text-xs text-muted-foreground mb-2 block font-medium">ระบุราคาสำหรับ {prize.display_name}</label>
                                    <div className="text-xs text-muted-foreground mb-1">ยอดรวมทั้งหมด: {filteredOrders.length} รายการ</div>
                                  </div>
                                </motion.div>
                              )}
                            </AnimatePresence>
                              ...
                              <AnimatePresence>
                                {getFilteredOrders(filteredOrders, prize.id).map((order, index) => (
                                  <motion.li
                                    ...
                                    className={`${order.isSpecialPattern ? 'bg-gray-50 dark:bg-red-900/30' : ''} ${!showRightPanel ? 'border-b border-border last:border-b-0' : ''}`}
                                    layout
                                    initial={{ opacity: 0, x: 24 }}
                                    ...
                                    variants={itemVariants}
                                  >
                                    {/* แสดงข้อมูลเลขและปุ่มลบ */}
                                    <div className="flex items-center justify-between px-3 py-1 hover:bg-muted/50">
                                      <div className="flex items-center gap-2">
                                      ...
                                      <div className="flex gap-1">
                                        {order.numbers.split('').map((num, i) => (
                                            <span key={i} className="w-7 h-7 flex items-center justify-center bg-muted text-foreground font-medium rounded-sm">{num}</span>
                                          ))}
                                            <AnimatePresence>
                                      {!showRightPanel && (
                                        <motion.div
                                          ...
                                          exit={{ opacity: 0, height: 0 }}
                                          transition={{ duration: 0.3, ease: "easeInOut" }}
                                          className="px-3 pb-3 bg-muted/20 overflow-hidden"
                                        >
                                          <div className="grid grid-cols-3 gap-2 text-xs w-82">
                                          <input
                                                type="number"
                                                placeholder="0"
                                                ...
                                                className="w-full px-2 py-1 text-sm text-bold border border-border rounded-md bg-background text-foreground focus:ring-1 focus:ring-red-500 focus:border-red-500"
                                              />
                                                <div className="w-full px-2 py-1 text-sm text-foreground font-medium text-gray-500"
                                               >x{(prize.prize_rate || 0).toLocaleString()}</div>
                                               <div className="w-full px-2 py-1 text-sm text-foreground font-medium text-red-600">
                                      ...
                                      <Button variant="ghost" size="icon" onClick={() => deleteOrder(order.id)} className="w-8 h-8 text-muted-foreground hover:text-red-500 ">
                                      <Trash2 size={16} />
                                    </Button>
                                    </div>
                                  </motion.li>
          ...
          {showRightPanel && (
            <motion.div 
              ...
              exit={{ opacity: 0, x: -50 }}
              transition={{ duration: 0.3, ease: "easeInOut" }}
              className="lg:col-span-2 bg-card text-card-foreground px-4 pb-6 rounded-lg shadow-md flex flex-col"
            >
            {/* Main Tabs */}
            <div className="flex justify-center border-b border-border w-full">
              <button onClick={() => { setActiveMainTab('manual'); setSelectedDigitForTwoPattern(''); setSelectedTwoDigitPattern(''); }} className={`py-1.5 px-3 text-md font-medium ${activeMainTab === 'manual' ? 'border-b-2 border-red-500 text-red-600' : 'text-muted-foreground hover:text-foreground'}`}>กดเลือกเอง</button>
              <button onClick={() => { setActiveMainTab('set'); setSelectedDigitForTwoPattern(''); setSelectedTwoDigitPattern(''); }} className={`py-1.5 px-3 text-md font-medium ${activeMainTab === 'set' ? 'border-b-2 border-red-500 text-red-600' : 'text-muted-foreground hover:text-foreground'}`}>ชุดตัวเลข</button>
            ...
            {/* Sub Tabs */}
            <div className="flex justify-center bg-muted p-0.5 rounded-md my-2">
              <button onClick={() => { setActiveDigitTab('three'); setCurrentInput(''); setSelectedDigitForTwoPattern(''); setSelectedTwoDigitPattern(''); }} className={`w-full py-1.5 px-2 text-xs rounded-md ${activeDigitTab === 'three' ? 'bg-background shadow font-semibold text-foreground' : 'text-muted-foreground'}`}>สามตัว</button>
              <button onClick={() => { setActiveDigitTab('two'); setCurrentInput(''); setSelectedDigitForTwoPattern(''); setSelectedTwoDigitPattern(''); }} className={`w-full py-1.5 px-2 text-xs rounded-md ${activeDigitTab === 'two' ? 'bg-background shadow font-semibold text-foreground' : 'text-muted-foreground'}`}>สองตัว</button>
              <button onClick={() => { setActiveDigitTab('run'); setCurrentInput(''); setSelectedDigitForTwoPattern(''); setSelectedTwoDigitPattern(''); }} className={`w-full py-1.5 px-2 text-xs rounded-md ${activeDigitTab === 'run' ? 'bg-background shadow font-semibold text-foreground' : 'text-muted-foreground'}`}>เลขวิ่ง</button>
            </div>
            ...
            {activeMainTab === 'set' ? (
              <div className="flex flex-col gap-2">
                ...
                <motion.div className="flex flex-row gap-1.5 mt-1.5 w-full" variants={containerVariants} initial="hidden" animate="visible">
                  {prizeInfo
                    ...
                    .map((prize) => (
                      <motion.div key={prize.id} variants={itemVariants}>
                        <Button
                          ...
                          className={cn(
                            "flex-1 w-full flex flex-row justify-center border-red-200 bg-red-50 text-red-900 dark:border-red-700 dark:bg-red-950 dark:text-red-200 h-12 text-xs gap-x-1 text-sm h-8",
                            selectedPrizeIds.includes(prize.id) ? "border-2 border-red-500 bg-red-100 dark:bg-red-900" : ""
                          )}
                          onClick={() => {
                ...
                <details className="mb-1.5" open={isMoreOptionsOpen} onToggle={(e) => setIsMoreOptionsOpen(e.currentTarget.open)}>
                  ...
                  <motion.div className="flex flex-row flex-wrap gap-1.5 mt-1" variants={containerVariants} initial="hidden" animate="visible">
                    {activeDigitTab === 'three' && [
                      ...
                      <motion.div key={txt} variants={itemVariants}>
                        <Button
                          key={txt}
                          variant="outline"
                          className={`text-sm h-8 ${selectedPattern === txt ? 'bg-red-100 border-red-500 dark:bg-red-900 dark:border-red-700' : ''} ${idx === 0 ? 'col-span-0' : ''}`}
                          onClick={() => {
                            if (selectedPrizeIds.length === 0) {
                      ...
                      <div className="flex flex-wrap gap-1 mb-1.5">
                        {Array.from({ length: 10 }, (_, i) => i * 100).map(base => (
                          <Button
                            ...
                            className={`h-7 w-12 text-[10px] font-mono px-0.5 ${threeDigitFilter === base ? 'border-red-400 bg-red-50 text-red-900 dark:border-red-700 dark:bg-red-950 dark:text-red-200' : ''}`}
              ...
              <>
                <div className="my-1.5 ">
                  ...
                  <motion.div className="flex flex-col-2 gap-1.5 mt-1.5 w-full" variants={containerVariants} initial="hidden" animate="visible">
                    {prizeInfo
                      .filter((prize) => prize.category === activeDigitTab)
                      .map((prize) => (
                        <motion.div key={prize.id} variants={itemVariants}>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[...'123456789'].map(n =>
                    <button key={n} onClick={() => handleNumberPress(n)} className="py-2 bg-background border border-border rounded-lg text-lg text-foreground hover:bg-muted focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 h-10 transition-all duration-150 active:bg-muted/80 active:scale-95">{n}</button>
                  )}
                  <button onClick={deleteLastOrder} className="py-2 bg-red-50 border-red-200 text-red-600 rounded-lg hover:bg-red-100 text-[10px] h-10 transition-all duration-150 active:bg-red-200 active:scale-95 dark:bg-red-950/50 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-950 dark:active:bg-red-900">ลบล่าสุด</button>
                  <button onClick={() => handleNumberPress('0')} className="py-2 bg-background border border-border rounded-lg text-lg text-foreground hover:bg-muted h-10">0</button>
                  <button onClick={handleBackspace} className="py-2 bg-red-50 border-red-200 text-red-600 rounded-lg hover:bg-red-100 flex items-center justify-center h-10 transition-all duration-150 active:bg-red-200 active:scale-95 dark:bg-red-950/50 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-950 dark:active:bg-red-900">
                    <ArrowLeft size={20} />
                  </button>
                </div>
                {/*<div className="flex items-center justify-between mt-3 border-t border-border pt-3">
          ...
          {!showRightPanel && (
            ...
            >
                  ...
                  {(() => {
                    ...
                    if (selectedIds.length > 1) {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-xl font-bold text-red-700 dark:text-red-200 tracking-tight mt-0.5">
                            รายการ {countStr}
                          </span>
                        </motion.div>
                      );
                    ...
                    if (orderToShow) {
                      ...
                      return orderToShow.numbers.split('').map((digit, index) => (
                        <div key={`${orderToShow?.id}-${index}`} className={`w-12 h-12 ${bgColor} rounded-lg flex items-center justify-center text-xl font-bold ${textColor}`}>
                          {digit}
                        </div>
                      ));
                    }
                    ...
                    return (
                      <>
                        <div className="w-12 h-12 bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center justify-center text-xl font-bold">-</div>
                        <div className="w-12 h-12 bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center justify-center text-xl font-bold">-</div>
                      </>
                    );
                  })()}
              ...
              <div className="grid grid-cols-3 gap-2 mb-6">
                {[5, 10, 20, 50, 100].map((price) => (
                  <Button
                    key={price}
                    variant="outline"
                    className="h-12 text-lg font-semibold border-2 hover:bg-red-50 hover:border-red-500 hover:text-red-600"
              ...
              {/* ช่องกรอกชื่อบิล */}
              <div className="mb-3">
                <label className="block text-sm font-medium text-foreground mb-1">ชื่อบิล/โพย (ไม่บังคับ)</label>
                <Input
                  type="text"
                  value={billName}
              ...
              <div className="mt-auto space-y-3">
                  <div className="text-sm space-y-2 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border dark:border-gray-800">
                      <div className="flex justify-between">
                          <span className="text-muted-foreground">ราคารวม</span>
                          <span className="font-semibold">{totalPayment.toLocaleString('en-US', { minimumFractionDigits: 2 })} ฿</span>
                      </div>
                      <div className="flex justify-between">
                          <span className="text-muted-foreground">เครดิตปัจจุบัน</span>
                          <span className="font-semibold">{credit?.toLocaleString('en-US', { minimumFractionDigits: 2 }) ?? 'N/A'} ฿</span>
                      </div>
                      <div className="flex justify-between border-t border-border pt-2 mt-2">
                          <span className="font-bold">เครดิตคงเหลือ</span>
                          <span className={`font-bold ${isCreditSufficient ? 'text-green-600' : 'text-red-600'}`}>
                              {creditAfterPayment?.toLocaleString('en-US', { minimumFractionDigits: 2 }) ?? 'N/A'} ฿
                          </span>
                      </div>
                  </div>
                  ...
                  <div className="mt-4">
                      <Button 
                          className="w-full bg-red-600 hover:bg-red-700 text-white h-12 text-lg font-semibold disabled:bg-gray-400 disabled:cursor-not-allowed"
                          disabled={orders.length === 0 || !isCreditSufficient || totalPayment === 0 || isSaving}
