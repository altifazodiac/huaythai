-- Add draw_time column to lottery_tickets table
ALTER TABLE public.lottery_tickets
ADD COLUMN draw_time TIME;

-- Update the handle_confirm_order function to accept and store draw_time
...
BEGIN
    ...
    -- 3. Create a new lottery ticket (now includes draw_time)
    INSERT INTO public.lottery_tickets (user_id, bill_name, bill_number, draw_date, draw_time, close_time, total_amount, status)
    VALUES (p_user_id, p_bill_name, p_bill_number, p_draw_date, p_draw_time::time, p_close_time, p_total_amount, 'confirmed')
